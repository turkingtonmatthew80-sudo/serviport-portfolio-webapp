...
export function ServicesSection() {
  const services = [
    ...
    },
    {
      ...
      description:
        "Estiba y desestiba con cuadrillas profesionales. Operamos grúas móviles y maquinaria de alta capacidad para cualquier tipo de carga.",
    },
    {
      ...
      description:
        "Protegemos su carga en nuestro propio Almacén General de Depósito para la nacionalización segura de mercancías.",
    },
    {
      ...
      description:
        "Intermediación de buques para el transporte de grandes volúmenes de carga o desarrollo de proyectos industriales especiales.",
    },
    {
      ...
      description:
        "Llevamos la mercancía desde o hacia el puerto con nuestra eficiente red logística de transporte en todo el territorio nacional.",
    },
    {
      ...
      description:
        "Atendemos de manera oportuna todas las necesidades de la tripulación y de la embarcación durante el atraque: bunkering, víveres, agua, etc.",
    },
  ];
  ...
  return (
    <section
      ref={containerRef}
      className="relative w-full bg-secondary py-12 md:py-16 lg:py-20 min-h-auto flex flex-col justify-center"
    >
      <div className="flex flex-col justify-center px-4 md:px-6 overflow-hidden h-full">
        {/* Background Graphic */}
        <div className="absolute top-0 right-0 w-1/2 h-full bg-primary/5 blur-[120px] rounded-full pointer-events-none transform translate-x-1/3 -translate-y-1/4" />

        <div className="max-w-[1260px] mx-auto w-full flex-1 flex flex-col justify-center relative z-10">
          <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 items-stretch pt-2">
            ...
            <div className="lg:w-1/2 w-full flex flex-col justify-center">
              ...
              >
                ...
                <p className="text-gray-300 text-sm leading-relaxed mb-3 md:mb-5 pb-3 md:pb-5 border-b border-white/10">
                  Serviport es tu socio logístico ideal en Venezuela. Nuestro
                  equipo de expertos brinda operaciones integrales desde la
                  atención en muelle y estiba, hasta la administración del
                ...
                {services.map((service, idx) => (
                  <Link
                    ...
                    className={`flex flex-col py-2 md:py-3 px-3 transition-all duration-300 cursor-pointer group rounded-lg ${
                      activeIndex === idx
                        ? "bg-background/10 shadow-md border-l-4 border-primary"
                        : "hover:bg-background/5 border-l-4 border-transparent"
                    }`}
                  >
                    <div className="flex items-center gap-2 md:gap-3">
                      <ArrowRight
                        size={16}
                        strokeWidth={2.5}
                        className={`transition-all duration-300 md:w-[18px] md:h-[18px] ${activeIndex === idx ? "text-primary translate-x-1.5" : "text-slate-500 group-hover:text-slate-400"}`}
                      />
                      <span
                        className={`text-sm md:text-lg font-bold transition-colors ${activeIndex === idx ? "text-white" : "text-slate-300 group-hover:text-white"}`}
                      >
                        {service.title}
                      </span>
                    </div>
              ...
              >
                <Link
                  to="/servicios"
                  className="group border-2 border-accent text-accent hover:text-white hover:bg-accent px-6 md:px-10 py-2.5 md:py-4 rounded font-bold transition-colors flex items-center justify-center gap-2 md:gap-3 text-center text-xs md:text-base uppercase tracking-wider w-fit mt-4 md:mt-6"
                >
            ...
            <div className="lg:w-1/2 w-full mt-4 lg:mt-0 aspect-square">
              <div className="w-full h-full relative overflow-hidden rounded-xl shadow-2xl border border-white/10 group cursor-pointer transition-colors duration-500 hover:border-primary/50">
