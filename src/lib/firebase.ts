...
export function handleFirestoreError(error: any, operation: OperationType, path: string) {
  if (error.code === 'unavailable' || error.message?.includes('offline')) {
    console.warn(`Firestore ${operation} offline error on path ${path}:`, error);
    throw new Error('No se pudo conectar a la base de datos. Asegúrate de haber habilitado Cloud Firestore en la consola de Firebase.');
  }
