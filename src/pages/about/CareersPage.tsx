...
export function CareersPage() {
  ...
  return (
    <div className="w-full bg-background-muted min-h-[100dvh]">
      ...
      <section className="bg-secondary text-white pt-24 pb-20 md:pb-32 px-4 md:px-6 relative overflow-hidden">
        ...
        <div className="max-w-[1260px] mx-auto relative z-10 text-center md:text-left">
          ...
          <p className="text-sm md:text-lg text-slate-300 font-medium max-w-3xl leading-relaxed mx-auto md:mx-0">
            Siempre estamos buscando talento portuario comprometido con el
            futuro de Venezuela.
          </p>
        </div>
      </section>
      ...
      <section className="py-12 md:py-16 lg:py-20 px-4 md:px-6 flex justify-center w-full bg-background">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-block px-6 py-2 bg-primary/10 text-primary font-bold tracking-wider text-sm uppercase rounded-sm mb-8 border border-primary/20">
            Nuestra Gente Es Nuestro Negocio
          </div>
          <h2 className="text-3xl md:text-5xl font-extrabold text-foreground mb-8 leading-tight">
            En Serviport Agentes Navieros hemos construido una fuerza laboral
            diversa y de alto rendimiento que trabaja colaborativamente.
          </h2>
          <p className="text-lg md:text-xl text-foreground-muted leading-relaxed font-medium">
            Valoramos a nuestra gente, y sabemos que su experiencia,
            profesionalismo y habilidades únicas nos han permitido estar donde
            estamos hoy. Seguimos creciendo en el mundo portuario y siempre
            estamos buscando talento apasionado por nuestra industria y por la
            entrega de un servicio logístico de excelencia, para llevar nuestras
            operaciones al siguiente nivel.
          </p>
        </div>
      </section>

      {/* AT A GLANCE */}
      <section className="py-12 md:py-16 lg:py-20 px-4 md:px-6 bg-secondary flex justify-center w-full">
        <div className="max-w-[1260px] mx-auto w-full">
          <div className="inline-block px-6 py-2 bg-primary/20 text-primary font-bold tracking-wider text-sm uppercase rounded-sm mb-12 border border-primary/30">
            De un Vistazo
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-12">
            {atAGlance.map((item, idx) => (
              <div
                key={idx}
                className="text-center md:text-left border-l-2 border-primary pl-6 py-2"
              >
                <div className="text-4xl md:text-6xl font-extrabold text-primary mb-2">
                  {item.number}
                </div>
                <div className="text-slate-300 font-bold tracking-wide">
                  {item.label}
                </div>
              </div>
            ))}
      ...
      <section className="py-12 md:py-16 lg:py-20 px-4 md:px-6 flex justify-center w-full bg-background-muted">
        <div className="max-w-[1260px] mx-auto w-full">
          <div className="inline-block px-6 py-2 bg-primary/10 text-primary font-bold tracking-wider text-sm uppercase rounded-sm mb-12 border border-primary/20">
            Nuestros Valores
          </div>
          <div className="grid md:grid-cols-3 gap-12">
            {values.map((val, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="bg-background p-8 rounded-sm shadow-sm border border-border hover:shadow-md transition-shadow"
                >
                  ...
                  <p className="text-foreground-muted leading-relaxed">{val.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="py-12 md:py-16 lg:py-20 px-4 md:px-6 flex justify-center w-full bg-secondary overflow-hidden relative">
        ...
        <div className="max-w-[1260px] mx-auto w-full relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="md:w-1/2 text-white">
            <h2 className="text-4xl md:text-6xl font-extrabold mb-6 leading-tight">
              Vacantes
            </h2>
            <p className="text-lg md:text-xl text-slate-300 mb-8 font-medium">
              ¿Estás interesado en trabajar con Serviport Agentes Navieros?
              Explora aquí nuestras vacantes operativas y corporativas actuales.
            </p>
            <Link
              to="/nosotros/carreras/vacantes"
              className="inline-flex items-center gap-3 bg-primary text-white font-bold py-4 px-8 rounded-sm hover:bg-primary transition-colors uppercase tracking-wider text-sm"
            >
